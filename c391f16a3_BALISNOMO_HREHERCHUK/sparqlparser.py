...
def getSQLFilter( filterLine ):
    ...
    # If the string contains REGEX, it is a regex filter.
    ...
    # If the string contains a math operator, it is a comparison filter
    ...
    if matchObj:
        ...
        # Strip the variable and the condition of quotations
        val1 = stripQuotes( val1 )
        val3 = stripQuotes( val3 )
        
        # Check if the variable or the condition are strings or variables.
        if val1.startswith('?'):
        ...
        # Return a string that calls our user made function
